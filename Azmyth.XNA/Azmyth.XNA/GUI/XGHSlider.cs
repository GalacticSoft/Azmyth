...
namespace XnaGUILib
{
    public class XGHSlider : XGControl
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                if (InputManager.PadHeld(PlayerIndex.One, Buttons.RightTrigger) || InputManager.PadHeld(PlayerIndex.One, Buttons.LeftTrigger))
                {
                    if (keyTimer <= 0.0f)
                    {
                        if (InputManager.PadHeld(PlayerIndex.One, Buttons.RightTrigger))
                        {
                            _value += 0.01f;
                            if (_value > 1.0f)
                                _value = 1.0f;
